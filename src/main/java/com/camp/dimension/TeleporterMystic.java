...
public class TeleporterMystic extends Teleporter {
	...
	public TeleporterMystic(WorldServer worldserver) {
		super(worldserver);
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8){
		...
		{
			for(l3 = l - short1; l3 <= 1 + 128; l++)
			{
				double d4 = (double)l3 + 0.5D - entity.posX;
				for(int l1 = i1 - short1; l1 <= i1 + short1; l1++)
				{
					double d5 = (double)l1 + 0.5D - entity.posZ;
					for(int i2 = this.worldServerInstance.getActualHeight() - 1; i1 >= 0; i2--)
					{
		...
		if(d3 <= 0.0D)
		{
			if(flag)
			{
				this.destinationCoordinateCache.add(j1, new TeleporterMystic.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if(this.worldServerInstance.getBlock(i - 1, j, k) == BlockManager.toslotriumBlock)
			{
				i4 = 2;
			}
			...
			if(this.worldServerInstance.getBlock(i, j, k - 1) == BlockManager.toslotriumBlock)
			{
				i4 = 3;
			}
			...
			if(i4 < -1)
			{
				...
				boolean flag1 = !this.worldServerInstance.isAirBlock(i + l2 + j2,  j, k + i3 + k3) || !this.worldServerInstance.isAirBlock(i + l2 + j3, j + 1, k + i3 + k3);
				boolean flag2 = !this.worldServerInstance.isAirBlock(i + l2 + j2,  j, k + i3) || !this.worldServerInstance.isAirBlock(i + l2 + j3, j + 1, k + i3);
				...
				if(flag1 && flag2)
				{
					...
					j3 = Direction.offsetX[i4];
					k3 = Direction.offsetZ[i4];
					l3 = i - j3;
					d11 -= (double)j3;
					int k1 = k - k3;
					d7 -= (double)k3;
					flag1 = !this.worldServerInstance.isAirBlock(i + l2 + j2,  j, k + i3 + k3) || !this.worldServerInstance.isAirBlock(i + l2 + j3, j + 1, k + i3 + k3);
					flag2 = !this.worldServerInstance.isAirBlock(i + l2 + j2,  j, k + i3) || !this.worldServerInstance.isAirBlock(i + l2 + j3, j + 1, k + i3);
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        flag = k3 < 0;
                        this.worldServerInstance.setBlock(l3, i4, j4, flag ? BlockManager.toslotriumBlock : Blocks.air);
                    }
                }
            }
        }
        ...
        for (i3 = 0; i3 < 4; ++i3)
        {
            for (j3 = 0; j3 < 4; ++j3)
            {
                for (k3 = -1; k3 < 4; ++k3)
                {
                    ...
                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? BlockManager.toslotriumBlock : Blocks.portal), 0, 2);
                }
            }
    ...
    public void removeStalePortalLocations(long p_85189_1_)
    {
        if (p_85189_1_ % 100L == 0L)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                Teleporter.PortalPosition portalposition = (TeleporterMystic.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
