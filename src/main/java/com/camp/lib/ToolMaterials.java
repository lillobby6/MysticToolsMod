package com.camp.lib;

import net.minecraft.item.Item.ToolMaterial;
import net.minecraftforge.common.util.EnumHelper;

public class ToolMaterials {
	public static ToolMaterial INFUSEDDIAMOND = EnumHelper.addToolMaterial("INFUSEDDIAMOND", 4, 8000, 16.0f, 5.0f, 20);
	public static ToolMaterial MYSTIC = EnumHelper.addToolMaterial("MYSTIC", 5, 12000, 20.0f, 7.0f, 30);
	public static ToolMaterial MAGIC = EnumHelper.addToolMaterial("MAGIC", 6, 15000, 30f, 10f, 30);
	public static ToolMaterial TERRIUM = EnumHelper.addToolMaterial("TERRIUM", 5, 3124, 12.5f, 4.0f, 17);
	public static ToolMaterial SKYIRON = EnumHelper.addToolMaterial("SKYIRON", 4, 600, 4f, 3.0f, 7);
	public static ToolMaterial BRIGHTFLAME = EnumHelper.addToolMaterial("BRIGHTFLAME", 6, 13000, 25.0f, 9.0f, 25);
	public static ToolMaterial MORTIUM = EnumHelper.addToolMaterial("MORTIUM", 4, 2161, 9.0F, 4.0F, 13);
	public static ToolMaterial VIVIDIUM = EnumHelper.addToolMaterial("VIVIDIUM", 4, 2161, 9.0F, 4.0F, 13);

}